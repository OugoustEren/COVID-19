...
```{r}
last_data <- data %>% group_by(tipo,`Province/State`,`Country/Region`) %>% filter(Date==max(Date))  
last_data_w <- data_w %>% group_by(`Province/State`,`Country/Region`) %>% filter(Date==max(Date))  
```
...
```{r}
tot_country <-last_data_w %>% group_by(`Country/Region`) %>% summarise_at(vars(confirmados:recuperados), sum) %>% arrange(desc(confirmados))
