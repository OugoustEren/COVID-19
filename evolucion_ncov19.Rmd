---
title: "Evolución de la epidemia de COVID-19"
subtitle: "<center> En España y el Mundo </center>"
output:
  html_document:
    keep_md: true
    highlight: tango
    theme: united
    toc: yes
    toc_float:
      collapsed: no
      smooth_scroll: no
    df_print: paged
    number_sections: true
---


```{r setup, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE}
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, error = FALSE, message = FALSE )
library(tidyverse)
library(broom)
library(plotly)
library(rgdal)
library(leaflet)
library(plotly)
library(DT)
```


*Última actualización:* `r Sys.Date()`

# Introducción 

Las fuentes de los datos utilizadas en este estudio son las siguientes: 

- Datos Internacionales: <https://github.com/CSSEGISandData/COVID-19> del repositorio de la John Hopkins University 

- Datos España: <https://github.com/datadista/datasets/tree/master/COVID%2019> del repositorio GitHub de **Datadista** 

Otros enlaces a dashboards o análisis interesantes son:

- [Dashboard John Hopkins](https://www.arcgis.com/apps/opsdashboard/index.html#/bda7594740fd40299423467b48e9ecf6)

- [Seguimiento El País](https://elpais.com/sociedad/2020/03/16/actualidad/1584360628_538486.html)

- [Ministerio de Sanidad España](https://covid19.isciii.es/)

- [Datos Italia por Provincias](https://github.com/pcm-dpc/COVID-19)




```{r}
# Internacionales 

confirmados_orig <- readr::read_csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_global.csv")

muertes_orig <- readr::read_csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv")

recuperados_orig <- readr::read_csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_recovered_global.csv")

```


```{r}
confirmados <- confirmados_orig %>% gather(key="Date",value="num",-(1:4)) %>% mutate(tipo="confirmados") %>% 
  select(tipo,everything()) %>% mutate(Date=as.Date(Date,format="%m/%d/%y"))

muertes <- muertes_orig %>% gather(key="Date",value="num",-(1:4)) %>% mutate(tipo="muertes") %>% 
  select(tipo,everything()) %>% mutate(Date=as.Date(Date,format="%m/%d/%y"))

recuperados <- recuperados_orig %>% gather(key="Date",value="num",-(1:4)) %>% mutate(tipo="recuperados") %>% 
  select(tipo,everything())%>% mutate(Date=as.Date(Date,format="%m/%d/%y"))


data <- bind_rows(confirmados, muertes)

data_w <- data  %>% pivot_wider(id_cols = c("Country/Region","Province/State","Date"), names_from = "tipo", values_from = "num")

# Añadimos recuperados
data <- bind_rows(data, recuperados)

data_w <- data_w %>% left_join(recuperados %>% select("Country/Region","Province/State","Date","num"), by=c("Country/Region","Province/State","Date")) %>% rename(recuperados=num)


#data$`Province/State`[is.na(data$`Province/State`)] <- data$`Country/Region`[is.na(data$`Province/State`)]
# Delete data from 2020-03-12. Same data as previous day
#data <- data %>% filter(Date!="2020-03-12")

data_w <- data_w %>% mutate(activos = confirmados - recuperados - muertes)
```


```{r}
last_data <- data %>% group_by(tipo,`Province/State`,`Country/Region`) %>% filter(!is.na(num)) %>%  filter(Date==max(Date))  
last_data_w <- data_w %>% group_by(`Province/State`,`Country/Region`) %>% filter(!is.na(confirmados)) %>%  filter(Date==max(Date))  
```


## Totales por país

```{r}
tot_country <- last_data %>% group_by(tipo,`Country/Region`) %>% summarise(num=sum(num)) %>% 
  pivot_wider(id_cols = c("Country/Region"), names_from = "tipo", values_from = "num") %>% 
  arrange(desc(confirmados))

# tot_country <-last_data_w %>% group_by(`Country/Region`,Date) %>% 
#   summarise_at(vars(confirmados:recuperados), ~sum(.,na.rm=TRUE)) %>% arrange(desc(confirmados))

# Calculamos tasas de mortalidad y recuperación 

tot_country<- tot_country %>% mutate(mort_rate = 100* muertes/confirmados, rec_rate=100* recuperados/confirmados)

#tot_country
datatable(tot_country) %>% formatRound(c("mort_rate", "rec_rate"),1)
```


# Evolución General


```{r}
ev_ctry <- data %>% group_by(`Country/Region`,Date, tipo) %>% summarise(num=sum(num, na.rm=TRUE))
ev_ctry_w <- data_w %>% group_by(`Country/Region`,Date) %>% summarise_at(.vars=vars(confirmados:activos),~sum(.,na.rm=TRUE))
new_ctry <- ev_ctry %>% group_by(`Country/Region`,tipo) %>% arrange(Date) %>% 
  mutate(num=num-lag(num), num=ifelse(is.na(num),0,num),incr=100*(num/lag(num)))
new_ctry_w <- ev_ctry_w %>% group_by(`Country/Region`) %>% arrange(Date) %>% 
  mutate_at(.vars=vars(confirmados:activos), .funs = ~(. - lag(.)))


```

## Casos Confirmados



### Acumulados

Escala lineal 

```{r}
sel.ctry = tot_country$`Country/Region`[1:12]

ggplot(ev_ctry %>% filter(`Country/Region` %in% sel.ctry, tipo=="confirmados")) + geom_line(aes(Date,num, group=1)) +
  facet_wrap(~`Country/Region`, scales="free_y") + labs(y="Confirmados") + theme(axis.text.x = element_text(angle=45, hjust = 1))
  
#scale_x_date(guide = guide_axis(n.dodge = 2)) 
```

Escala logarítmica 

```{r}
ggplot(ev_ctry %>% filter(`Country/Region` %in% sel.ctry, tipo=="confirmados")) + geom_line(aes(Date,num, group=1)) +
  facet_wrap(~`Country/Region`, scales="free_y") + 
  labs(y="Confirmados") + theme(axis.text.x = element_text(angle=45, hjust = 1))+ 
  scale_y_log10()

```

### Nuevos casos

```{r}
ggplot(new_ctry %>% filter(`Country/Region` %in% sel.ctry, tipo=="confirmados")) + geom_line(aes(Date,num, group=1)) +
  facet_wrap(~`Country/Region`, scales="free_y") + 
  labs(y="Confirmados") + theme(axis.text.x = element_text(angle=45, hjust = 1))

```



## Casos activos (Confirmados - Recuperados)

```{r}
ggplot(ev_ctry_w %>% filter(`Country/Region` %in% sel.ctry, Date< Sys.Date()-1)) + geom_line(aes(Date,activos, group=1)) +
  facet_wrap(~`Country/Region`, scales="free_y") + theme(axis.text.x = element_text(angle=60, hjust = 1))
```

Log 

```{r}
ggplot(ev_ctry_w %>% filter(`Country/Region` %in% sel.ctry, activos>0)) + geom_line(aes(Date,activos, group=1)) +
  facet_wrap(~`Country/Region`, scales="free_y") + scale_y_log10()

```

Nuevos casos Diarios - Recuperados

```{r}
ggplot(new_ctry_w %>% filter(`Country/Region` %in% sel.ctry, activos>0)) + geom_line(aes(Date,activos, group=1)) +
  facet_wrap(~`Country/Region`, scales="free_y") + labs(y="Nuevos Casos - Nuevos Recuperados") + 
  theme(axis.text.x = element_text(angle = 30, hjust = 1))

```


## Fallecidos

### Acumulados

```{r}
ggplot(ev_ctry_w %>% filter(`Country/Region` %in% sel.ctry)) + geom_line(aes(Date,muertes, group=1)) +
  facet_wrap(~`Country/Region`, scales="free_y")

```

```{r}
ggplot(ev_ctry_w %>% filter(`Country/Region` %in% sel.ctry)) + geom_line(aes(Date,muertes, group=1)) +
  facet_wrap(~`Country/Region`, scales="free_y") + scale_y_log10()

```

### Nuevos Fallecidos

```{r}
ggplot(new_ctry_w %>% filter(`Country/Region` %in% sel.ctry)) + geom_line(aes(Date,muertes, group=1)) +
  facet_wrap(~`Country/Region`, scales="free_y")

```


# Modelado de la Evolución en Paises Seleccionados


## Casos

```{r}
sel_ctrys <- c("Spain", "Italy", "France", "US", "Germany", "United Kingdom", "Portugal")
date_ini = "2020-03-02"

data_casos = ev_ctry %>% filter(`Country/Region` %in% sel_ctrys, Date >= date_ini , tipo=="confirmados")

p <- ggplot(data_casos) + geom_point(aes(Date,num, color=`Country/Region`)) + 
  geom_line(aes(Date,num, color=`Country/Region`), size=0.25,alpha=0.5) + 
  scale_color_brewer(palette="Set1")#

ggplotly(p, dynamicTicks = TRUE)

```

Escala logarítmica

```{r}
p <- ggplot(data_casos %>% filter(num>1)) + geom_point(aes(Date,num, color=`Country/Region`)) + 
  geom_line(aes(Date,num, color=`Country/Region`), size=0.25,alpha=0.2) + 
  scale_y_log10() +
  scale_color_brewer(palette="Set1")
#ggplotly(p)


fig <- plot_ly(data = data_casos %>% ungroup(), x = ~Date, y = ~num, 
               group = ~`Country/Region`,  color= ~`Country/Region`, mode='lines+markers',  type="scatter")
fig <- layout(fig,  yaxis = list(type = "log"))
fig


```

Nuevos casos diarios



```{r}
data_new_casos = new_ctry %>% filter(`Country/Region` %in% sel_ctrys, Date >= date_ini , tipo=="confirmados")

p <- ggplot(data_new_casos) + geom_point(aes(Date,num, color=`Country/Region`)) + 
  geom_line(aes(Date,num, color=`Country/Region`), size=0.25,alpha=0.5) + 
  scale_color_brewer(palette="Set1")#

ggplotly(p, dynamicTicks = TRUE)

```

Casos diarios: Nuevos confirmados - Nuevos recuperados

```{r}
ggplot(new_ctry_w %>% filter(`Country/Region` %in% sel_ctrys, activos>0, Date>"2020-03-01")) + 
  geom_line(aes(Date,activos, group=1), alpha=0.7) +
  geom_smooth(aes(Date,activos, color=`Country/Region`), alpha=1, size=0.75, se=FALSE) +  
  facet_wrap(~`Country/Region`, scales="free_y") + labs(y="Nuevos Casos - Nuevos Recuperados (diarios)") + 
  theme(axis.text.x = element_text(angle = 30, hjust = 1)) + theme(legend.position = "none") +
  scale_color_brewer(palette = "Set1")
```


### Ajustes Exponenciales

```{r}
#cut_fit_date = as.Date("2020-03-15")
cut_fit_date <-  Sys.Date() - 5
data_casos <- data_casos %>% group_by(`Country/Region`) %>% mutate(t=as.numeric(Date-min(Date)))

data_casos_nest <- data_casos %>% nest(-`Country/Region`) %>% group_by(`Country/Region`) %>%  
  mutate(
    exp_mod_full = purrr::map(data, ~lm(log(num)~t, data= .)),
    exp_mod_a = purrr::map(data, ~lm(log(num)~t, data= filter(., Date>=cut_fit_date))),
    exp_mod_b = purrr::map(data, ~lm(log(num)~t, data= filter(., Date<cut_fit_date)))
    )

data_casos_nest <- data_casos_nest %>% mutate(tidied_full = purrr::map(exp_mod_full,broom::tidy),
                                              tidied_a = purrr::map(exp_mod_a,broom::tidy), 
                                              tidied_b = purrr::map(exp_mod_b,broom::tidy))




```


Fit periodo completo

```{r}
(coefs_full <- data_casos_nest %>% unnest(tidied_full) %>% select(`Country/Region`,term, estimate, std.error) %>% filter(term=="t") %>% 
  mutate(dupl_t = log(2)/estimate))
```

Fit después de  `r as.character(cut_fit_date)`


```{r}
(coefs_a <- data_casos_nest %>% unnest(tidied_a) %>% select(`Country/Region`,term, estimate, std.error) %>% filter(term=="t") %>% 
  mutate(dupl_t = log(2)/estimate))

```


Fit antes de   `r as.character(cut_fit_date)`


```{r}
(coefs_b <- data_casos_nest %>% unnest(tidied_b) %>% select(`Country/Region`,term, estimate, std.error) %>% filter(term=="t") %>% 
  mutate(dupl_t = log(2)/estimate))

```

Plot fits antes/despúes `r cut_fit_date` y tiempos de duplicación 

```{r, fig.width=10, fig.height=6}

tmp =data_casos_nest %>% 
  mutate(fitted_full=map(exp_mod_full,augment),fitted_a=map(exp_mod_a,augment),fitted_b=map(exp_mod_b,augment))
  
  
tmp_full <- tmp %>% unnest(fitted_full) %>% mutate(fitted=exp(.fitted)) %>% 
  select(`Country/Region`,t, fitted) %>%  mutate(Date=min(data_casos$Date) + t)

tmp_a <- tmp %>% unnest(fitted_a) %>% mutate(fitted=exp(.fitted)) %>% 
  select(`Country/Region`,t, fitted) %>%  mutate(Date=min(data_casos$Date) + t)

tmp_b <- tmp %>% unnest(fitted_b) %>% mutate(fitted=exp(.fitted)) %>% 
  select(`Country/Region`,t, fitted) %>%  mutate(Date=min(data_casos$Date) + t)




# Prediccion con el modelo after

future = crossing(data.frame(Date = seq(Sys.Date(),as.Date("2020-04-15"), by="days")),data.frame(Country=sel_ctrys))

future <- future %>% mutate(t=as.numeric(Date-min(data_casos$Date)))


future <- future %>% group_by(Country) %>% do({
  df=.
  ctry=df$Country[1]
  #cat(ctry,"\n")
  mod = data_casos_nest$exp_mod_a[data_casos_nest$`Country/Region`== ctry][[1]]
  prdf <- predict.lm(mod,newdata = df,interval = "prediction")
  prdf <- as.data.frame(prdf) %>% mutate_all(exp)
  df <- bind_cols(df, prdf %>% dplyr::rename(pred=fit))
  df
  df
})

dupl_index_info_a <- tmp_a %>% left_join(coefs_a, by="Country/Region")  %>% 
  group_by(`Country/Region`) %>% filter(t==max(t)) %>% ungroup()

dupl_index_info_b <- tmp_b %>% left_join(coefs_b, by="Country/Region")  %>% 
  group_by(`Country/Region`) %>% filter(t==min(t)) %>% ungroup()

  
ggplot(data_casos) + 
  geom_point(aes(Date,num, color=`Country/Region`))  +
  geom_ribbon(aes(Date,ymin=lwr, ymax=upr, group=Country), color="grey90", alpha=0.05,data=future) +
  geom_line(aes(Date,fitted, color=`Country/Region`), data=tmp_b) + 
  geom_line(aes(Date,fitted, color=`Country/Region`), data=tmp_a) +
  geom_line(aes(Date,fitted, color=`Country/Region`), data=tmp_full, linetype=3, alpha=1) +
  geom_line(aes(Date,pred, color=Country), linetype = 2, data=future) +
  geom_vline(aes(xintercept = cut_fit_date), col = "white", size=3, alpha=0.7) +
  ggrepel::geom_label_repel(aes(Date + 3, fitted*2, label=round(dupl_t,1), 
                            color=`Country/Region`), fill="grey90", size=3 , alpha=1, data=dupl_index_info_a, segment.alpha = 0) +
  ggrepel::geom_label_repel(aes(Date + 1, fitted*1.4, label=round(dupl_t,1), color=`Country/Region`), 
                            size=3,  fill="grey90", alpha=0.8, data=dupl_index_info_b, segment.alpha = 0) +
  geom_text(aes(x=cut_fit_date-4, y = 4e5, label=paste("Fits after and before",cut_fit_date,"\n Labels: duplication times in days")), size=3) +
  scale_y_log10() + labs(x="",y="Casos Confirmados + Predicción (log10)") +
  scale_color_brewer(palette="Set1")

```

### Tiempo duplicación dinámico

```{r}
fit_window=7 # days
first_date = as.Date("2020-03-07")

fit_dates <- unique(data_casos$Date[data_casos$Date>=first_date])

din_dupl <- data.frame(fdate= fit_dates) %>% group_by(fdate) %>% do({
    fd = .$fdate
    df <- data_casos %>%filter(Date<=fd, Date>fd-fit_window)
    res <- df %>% nest(-`Country/Region`) %>% group_by(`Country/Region`) %>%  
      mutate(exp_mod = purrr::map(data, ~lm(log(num)~t, data= .))) %>%
      mutate(tidied = purrr::map(exp_mod,broom::tidy)) %>% 
      unnest(tidied) %>% select(`Country/Region`,term, estimate, std.error) %>% filter(term=="t") %>% 
        mutate(dupl_t = log(2)/estimate)
})

ggplot(din_dupl) + geom_point(aes(fdate,dupl_t, color=`Country/Region`)) +
  geom_smooth(aes(fdate,dupl_t, color=`Country/Region`), se=FALSE, size=0.5, alpha=0.5, span=0.5) +
    scale_color_brewer(palette="Set1") + labs(y="Tiempo de duplicación de casos (dias)", title="Tiempos de duplicacion en ventana movil de una semana antes de la fecha")

```



## Fallecidos


```{r}
date_ini <- "2020-03-01"

data_muertes = ev_ctry %>% filter(`Country/Region` %in% sel_ctrys, Date>=date_ini , tipo == "muertes")


p <- ggplot(data_muertes) + geom_point(aes(Date,num, color=`Country/Region`))  +
  geom_line(aes(Date,num, color=`Country/Region`)) +
  scale_color_brewer(palette="Set1")#

plotly::ggplotly(p)



```

```{r}
fig <- plot_ly(data = data_muertes %>% ungroup(), x = ~Date, y = ~num, 
               group = ~`Country/Region`,  color= ~`Country/Region`, mode='lines+markers',  type="scatter")
fig <- layout(fig,  yaxis = list(type = "log"))
fig
```

Fallecidos diarios

```{r}
ggplot(new_ctry_w %>% filter(`Country/Region` %in% sel_ctrys, activos>0, Date>"2020-03-01")) + 
  geom_line(aes(Date,muertes, group=1), alpha=0.7) +
  geom_smooth(aes(Date,muertes, color=`Country/Region`), alpha=1, size=0.75, se=FALSE) +  
  facet_wrap(~`Country/Region`, scales="free_y") + labs(y="Nuevos Fallecidos (diarios)") + 
  theme(axis.text.x = element_text(angle = 30, hjust = 1)) + theme(legend.position = "none") +
  scale_color_brewer(palette = "Set1")
```



### Ajustes Exponenciales

```{r}
#cut_fit_date = as.Date("2020-03-15")
cut_fit_date = Sys.Date() - 5

data_muertes <- data_muertes %>% group_by(`Country/Region`) %>% mutate(t=as.numeric(Date-min(Date))) %>% filter(num>0)

data_muertes_nest <- data_muertes %>% nest(-`Country/Region`) %>% group_by(`Country/Region`) %>%  
  mutate(
    exp_mod_full = purrr::map(data, ~lm(log(num)~t, data= .)),
    exp_mod_a = purrr::map(data, ~lm(log(num)~t, data= filter(., Date>=cut_fit_date))),
    exp_mod_b = purrr::map(data, ~lm(log(num)~t, data= filter(., Date<cut_fit_date)))
    )

data_muertes_nest <- data_muertes_nest %>% mutate(tidied_full = purrr::map(exp_mod_full,broom::tidy),
                                              tidied_a = purrr::map(exp_mod_a,broom::tidy), 
                                              tidied_b = purrr::map(exp_mod_b,broom::tidy))




```


Fit periodo completo

```{r}
(coefsm_full <- data_muertes_nest %>% unnest(tidied_full) %>% select(`Country/Region`,term, estimate, std.error) %>% filter(term=="t") %>% 
  mutate(dupl_t = log(2)/estimate))
```

Ajuste después de  `r as.character(cut_fit_date)`


```{r}
(coefsm_a <- data_muertes_nest %>% unnest(tidied_a) %>% select(`Country/Region`,term, estimate, std.error) %>% filter(term=="t") %>% 
  mutate(dupl_t = log(2)/estimate))

```


Ajuste antes de `r as.character(cut_fit_date)`


```{r}
(coefsm_b <- data_muertes_nest %>% unnest(tidied_b) %>% select(`Country/Region`,term, estimate, std.error) %>% filter(term=="t") %>% 
  mutate(dupl_t = log(2)/estimate))

```

Plot fits and future projections

```{r, fig.width=10, fig.height=6}

tmp =data_muertes_nest %>% 
  mutate(fitted_full=map(exp_mod_full,augment),fitted_a=map(exp_mod_a,augment),fitted_b=map(exp_mod_b,augment))
  
  
tmp_full <- tmp %>% unnest(fitted_full) %>% mutate(fitted=exp(.fitted)) %>% 
  select(`Country/Region`,t, fitted) %>%  mutate(Date=min(data_muertes$Date) + t)

tmp_a <- tmp %>% unnest(fitted_a) %>% mutate(fitted=exp(.fitted)) %>% 
  select(`Country/Region`,t, fitted) %>%  mutate(Date=min(data_muertes$Date) + t)

tmp_b <- tmp %>% unnest(fitted_b) %>% mutate(fitted=exp(.fitted)) %>% 
  select(`Country/Region`,t, fitted) %>%  mutate(Date=min(data_muertes$Date) + t)




# Prediccion con el modelo after

future = crossing(data.frame(Date = seq(Sys.Date(),as.Date("2020-04-15"), by="days")),data.frame(Country=sel_ctrys))

future <- future %>% mutate(t=as.numeric(Date-min(data_muertes$Date)))


future <- future %>% group_by(Country) %>% do({
  df=.
  ctry=df$Country[1]
  #cat(ctry,"\n")
  mod = data_muertes_nest$exp_mod_a[data_muertes_nest$`Country/Region`== ctry][[1]]
  prdf <- predict.lm(mod,newdata = df,interval = "prediction")
  prdf <- as.data.frame(prdf) %>% mutate_all(exp)
  df <- bind_cols(df, prdf %>% dplyr::rename(pred=fit))
  df
  df
})

dupl_index_info_a <- tmp_a %>% left_join(coefsm_a, by="Country/Region")  %>% 
  group_by(`Country/Region`) %>% filter(t==max(t)) %>% ungroup()

dupl_index_info_b <- tmp_b %>% left_join(coefsm_b, by="Country/Region")  %>% 
  group_by(`Country/Region`) %>% filter(t==min(t)) %>% ungroup()

  
ggplot(data_muertes) + 
  geom_point(aes(Date,num, color=`Country/Region`))  +
  geom_ribbon(aes(Date,ymin=lwr, ymax=upr, group=Country), color="grey90", alpha=0.05,data=future) +
  geom_line(aes(Date,fitted, color=`Country/Region`), data=tmp_b) + 
  geom_line(aes(Date,fitted, color=`Country/Region`), data=tmp_a) +
  geom_line(aes(Date,fitted, color=`Country/Region`), data=tmp_full, linetype=3, alpha=1) +
  geom_line(aes(Date,pred, color=Country), linetype = 2, data=future) +
  geom_vline(aes(xintercept = cut_fit_date), col = "white", size=3, alpha=0.5) +
  ggrepel::geom_label_repel(aes(Date + 3, fitted*2, label=round(dupl_t,1), 
                            color=`Country/Region`), fill="grey90", size=3 , alpha=1, data=dupl_index_info_a, segment.alpha = 0) +
  ggrepel::geom_label_repel(aes(Date + 1, fitted*1.4, label=round(dupl_t,1), color=`Country/Region`), 
                            size=3,  fill="grey90", alpha=0.8, data=dupl_index_info_b, segment.alpha = 0) +
  geom_text(aes(x=cut_fit_date-4, y = 5e4, label=paste("Fits after and before" ,cut_fit_date,"\n Labels: duplication times in days")), size=3) +
  scale_y_log10() + labs(x="",y="Fallecidos  + Proyección (log10)") +
  scale_color_brewer(palette="Set1") +  
  coord_cartesian(ylim=c(1, 7e4))


```

### Tiempo duplicación dinámico

```{r}
fit_window=7 # days
first_date = as.Date("2020-03-07")

# Exclude Germany
#data_muertes_sger <- data_muertes %>% filter(`Country/Region`!="Germany")

fit_dates <- unique(data_muertes$Date[data_muertes$Date>=first_date])

din_dupl_muertes <- data.frame(fdate= fit_dates) %>% group_by(fdate) %>% do({
    fd = .$fdate
    df <- data_muertes %>%filter(Date<=fd, Date>fd-fit_window)
    res <- df %>% nest(-`Country/Region`) %>% group_by(`Country/Region`) %>%  
      mutate(exp_mod = purrr::map(data, ~lm(log(num)~t, data= .))) %>%
      mutate(tidied = purrr::map(exp_mod,broom::tidy)) %>% 
      unnest(tidied) %>% select(`Country/Region`,term, estimate, std.error) %>% filter(term=="t") %>% 
        mutate(dupl_t = log(2)/estimate)
}) %>% mutate(dupl_t=ifelse(dupl_t>10,NA,dupl_t))

ggplot(din_dupl_muertes) + geom_point(aes(fdate,dupl_t, color=`Country/Region`)) +
  geom_smooth(aes(fdate,dupl_t, color=`Country/Region`), se=FALSE, size=0.5, alpha=0.5, span=0.5) +
    scale_color_brewer(palette="Set1") + labs(y="Tiempo de duplicación de fallecidos (dias)", title="Tiempos de duplicacion en ventana movil de una semana antes de la fecha")

```


## Visualización desde el instante con 100 casos

### Casos

```{r}
sel_ctrys <- c("Spain", "Italy", "France", "US", "Germany", "United Kingdom")
#sel_ctrys = tasas_tot$`Country/Region`[1:6]

nini=100

data_casos_icom <-  ev_ctry %>% filter(`Country/Region` %in% sel_ctrys, num>= nini, tipo=="confirmados")
data_casos_icom <- data_casos_icom %>% group_by(`Country/Region`) %>% mutate(ini_Date = min(Date), t=as.numeric(Date-min(Date)))


#Rescale to begin exactly from nini
#data_casos_icom <- data_casos_icom %>% group_by(`Country/Region`) %>% mutate(num=nini*num/num[1]) %>% filter(t<30)


ggplot(data_casos_icom %>% na.omit()) + geom_point(aes(t,num, color=`Country/Region`)) + 
  geom_line(aes(t,num, color=`Country/Region`), size=0.1, alpha=0.5) + scale_y_log10() + 
  labs(x="dias desde alcanzar 100 infectados", y="Casos Confimados") + scale_color_brewer(palette = "Set1")

# ggplot(data_casos_icom %>% na.omit()) + geom_point(aes(t,num, color=`Country/Region`)) +
#   geom_line(aes(t,num, color=`Country/Region`), size=0.1, alpha=0.5)  + labs(x="dias desde alcanzar 100 infectados", y="Casos Confimados") +   
#   scale_color_brewer(palette = "Set1")

```


Incluyendo China


```{r}
sel_ctrys2 <- c("Spain", "Italy", "France", "US", "Germany", "United Kingdom", "China")
#sel_ctrys = tasas_tot$`Country/Region`[1:6]

nini=500

data_casos_icom2 <-  ev_ctry %>% filter(`Country/Region` %in% sel_ctrys2, num>= nini, tipo=="confirmados")
data_casos_icom2 <- data_casos_icom2 %>% group_by(`Country/Region`) %>% mutate(ini_Date = min(Date), t=as.numeric(Date-min(Date)))

#Rescale to begin exactly from nini
#data_casos_icom <- data_casos_icom %>% group_by(`Country/Region`) %>% mutate(num=nini*num/num[1]) %>% filter(t<30)


ggplot(data_casos_icom2 %>% na.omit()) + geom_point(aes(t,num, color=`Country/Region`)) + 
  geom_line(aes(t,num, color=`Country/Region`), size=0.1, alpha=0.5) + scale_y_log10() + 
  coord_cartesian(xlim=c(0,50)) +
  labs(x="Dias desde alcanzar 100 infectados", y="Casos Confimados") + scale_color_brewer(palette = "Set1")


```


### Fallecidos 


```{r}
sel_ctrys2 <- c("Spain", "Italy", "France", "US", "Germany", "United Kingdom", "China")
#sel_ctrys = tasas_tot$`Country/Region`[1:6]



data_deaths_icom2 <-  ev_ctry %>% filter(`Country/Region` %in% sel_ctrys2, tipo=="muertes")

# Select the first date selected for casos
first_date_casos <- data_casos_icom2 %>% group_by(`Country/Region`) %>%  filter(t==min(t))

data_deaths_icom2 <- data_deaths_icom2 %>% left_join(first_date_casos %>% select(`Country/Region`, Date), by= "Country/Region", suffix=c("",".ini"))
data_deaths_icom2 <- data_deaths_icom2 %>% filter(Date>Date.ini)

data_deaths_icom2 <- data_deaths_icom2 %>% group_by(`Country/Region`) %>% mutate(ini_Date = min(Date), t=as.numeric(Date-min(Date)))



#Rescale to begin exactly from nini
#data_casos_icom <- data_casos_icom %>% group_by(`Country/Region`) %>% mutate(num=nini*num/num[1]) %>% filter(t<30)


ggplot(data_deaths_icom2 %>% na.omit()) + geom_point(aes(t,num, color=`Country/Region`)) + 
  geom_line(aes(t,num, color=`Country/Region`), size=0.1, alpha=0.5) + scale_y_log10() + 
  labs(x="dias desde alcanzar 100 infectados", y="Fallecidos") + scale_color_brewer(palette = "Set1") +
  coord_cartesian(xlim=c(0,50))

```




# España. Detalles por CCAA

```{r, message=FALSE}
# España CCAA
url_base <- "https://raw.githubusercontent.com/datadista/datasets/master/COVID%2019/"
confirmados_ccaa <- readr::read_csv(paste0(url_base,"/ccaa_covid19_casos.csv"))
muertes_ccaa <- readr::read_csv(paste0(url_base,"/ccaa_covid19_fallecidos.csv"))
recuperados_ccaa <- readr::read_csv(paste0(url_base,"/ccaa_covid19_altas.csv"))
uci_ccaa <- readr::read_csv(paste0(url_base,"/ccaa_covid19_uci.csv"))

```

```{r}
data_ccaa <- bind_rows(
  confirmados_ccaa %>% gather(key="Date",value="num",-(1:2)) %>% mutate(tipo="confirmados") %>% select(tipo,everything()),
  muertes_ccaa %>% gather(key="Date",value="num",-(1:2)) %>% mutate(tipo="muertes") %>% select(tipo,everything()),
  recuperados_ccaa %>% gather(key="Date",value="num",-(1:2)) %>% mutate(tipo="recuperados") %>% select(tipo,everything()),
  uci_ccaa %>% gather(key="Date",value="num",-(1:2)) %>% mutate(tipo="uci") %>% select(tipo,everything())
)

data_ccaa <- data_ccaa %>% mutate(Date=as.Date(Date,format="%Y-%m-%d"))

```


```{r}
#poblacion CCAA
pob_ccaa <- read.table("https://www.ine.es/jaxiT3/files/t/es/csv_bdsc/2915.csv",sep=";", encoding = "UTF-8", 
                       col.names = c("CCAA","mun_size","periodo","pob"),header=TRUE,stringsAsFactors = FALSE)

pob_ccaa<- pob_ccaa %>% filter(mun_size=="Total",periodo==max(periodo))
# Convertir pob a numeric
pob_ccaa<- pob_ccaa %>% mutate(pob= as.numeric(gsub("\\.","",pob)))

# Dividir nombre CCAA en código y nombre
pob_ccaa<- pob_ccaa %>% separate(CCAA, c("cod_ine", "CCAA"), " ")
pob_ccaa<- pob_ccaa %>% mutate(CCAA= gsub(",","",CCAA))
pob_ccaa$CCAA[pob_ccaa$cod_ine=="Total"]="Total"
pob_ccaa$cod_ine[pob_ccaa$cod_ine=="Total"]="00"


```


```{r, results='hide'}
# shp file CCAA
shp_ccaa <- rgdal::readOGR(dsn="ccaa_shp",layer = "comunidades-autonomas-espanolas", stringsAsFactors = FALSE, encoding = "UTF-8", use_iconv = TRUE)
names(shp_ccaa)[1]="cod_ine"

```

## Totales 

```{r}
tot_ccaa <- data_ccaa %>% group_by(cod_ine,CCAA, tipo) %>% filter(Date==max(Date), !(CCAA %in% c("Ceuta","Melilla"))) %>% 
  summarise(Date=max(Date), num=sum(num)) 

tot_ccaa <- tot_ccaa %>% pivot_wider(id_cols = c("cod_ine","CCAA","Date"), names_from = "tipo", values_from ="num") %>% arrange(-confirmados)

tot_ccaa <- tot_ccaa %>% left_join(pob_ccaa %>% select(cod_ine,pob), by="cod_ine") %>% 
  mutate(confirmados_phab = 1e5*confirmados/pob,muertes_phab=1e5*muertes/pob, mort_rate=100*muertes/confirmados, rec_rate=100*recuperados/confirmados)

datatable(tot_ccaa %>% ungroup() %>% select(-cod_ine, -Date), rownames = FALSE)  %>% formatRound(c(7:10),1)
```


##  Casos


### Mapa 

```{r}
shpmap <- merge(shp_ccaa,tot_ccaa, by=c("cod_ine")) 

pal_casos <- colorNumeric("YlOrRd",shpmap$confirmados_phab,
                        na.color = "transparent")


popup_dat <- paste0("<strong>CCAA: </strong>", 
                            shpmap$CCAA,
                            "<br><strong>Casos: </strong>", 
                            round(shpmap$confirmados,0), 
                            "<br><strong>Casos x 100000 hab </strong>",
                            round(shpmap$confirmados_phab,1),
                            "<br><strong>Fallecidos: </strong>", 
                            round(shpmap$muertes,0), 
                            "<br><strong>Fallecidos x 100000 hab </strong>",
                            round(shpmap$muertes_phab,1))


leaflet(data=shpmap) %>%
  addTiles() %>% 
  #setView(lng=-1.5, lat=39.85, zoom = 06) %>% 
  addPolygons(fillColor = ~pal_casos(confirmados_phab),
              stroke = TRUE, color="grey",weight = 1, fillOpacity =0.8, popup = popup_dat) %>% 
  leaflet::addLegend(position="bottomright", title="Casos Confirmados <br> x 100000 hab", pal=pal_casos,values = shpmap$confirmados_phab)

```



### Evolución casos

```{r}
data_ccaa <- data_ccaa %>% left_join(pob_ccaa %>% select(cod_ine,pob), by="cod_ine") %>% mutate(num_phab = 1e5*num/pob)
data_new_ccaa <- data_ccaa %>% group_by(CCAA,tipo) %>% arrange(Date) %>% mutate(num=num-lag(num)) %>%  mutate(num_phab = 1e5*num/pob)
```

Acumulados

```{r}
ggplot(data_ccaa %>% filter(tipo=="confirmados")) + geom_line(aes(Date, num_phab)) + facet_wrap(~CCAA)

```

Nuevos

```{r}
ggplot(data_new_ccaa %>% filter(tipo=="confirmados")) + geom_line(aes(Date, num_phab)) + facet_wrap(~CCAA, scales="free_y") +
  theme(axis.text.x = element_text(angle=30,hjust = 1))

```



```{r}
sel_ccaa = c(tot_ccaa$cod_ine[1:8],"02")
sel_ccaa <- sel_ccaa[sel_ccaa!="00"]

data_casos_ccaa = data_ccaa %>% filter(Date>= date_ini , tipo=="confirmados", (cod_ine %in% sel_ccaa))
tot_casos_sel <- tot_ccaa %>% filter(cod_ine %in% sel_ccaa)

data_new_casos_ccaa = data_new_ccaa %>% filter(Date>= date_ini , tipo=="confirmados", (cod_ine %in% sel_ccaa))
last_new_ccaa <- data_new_casos_ccaa %>% filter(Date==max(Date))

data_new_altas_ccaa <- data_new_ccaa %>% filter(Date>= date_ini , tipo=="recuperados", (cod_ine %in% sel_ccaa))
last_new_altas_ccaa <- data_new_altas_ccaa %>% filter(Date==max(Date))

data_new_muertes_ccaa <- data_new_ccaa %>% filter(Date>= date_ini , tipo=="muertes", (cod_ine %in% sel_ccaa))
last_new_muertes_ccaa <- data_new_muertes_ccaa %>% filter(Date==max(Date))

data_new_all_ccaa <- data_new_casos_ccaa %>% select(CCAA,Date,confirmados=num, confirmados_phab=num_phab) %>% 
  left_join(data_new_altas_ccaa %>% select(CCAA,Date,altas=num, altas_phab=num_phab), by=c("CCAA","Date"))

data_new_all_ccaa <- data_new_all_ccaa %>% mutate(activos=confirmados-altas,activos_phab=confirmados_phab-altas_phab)

# If we consider more than 9 CCAA 
# getPalette = colorRampPalette(RColorBrewer::brewer.pal(9, "Set1"))


p <- ggplot(data_casos_ccaa) + geom_point(aes(Date,num_phab, color=CCAA)) + 
  geom_line(aes(Date,num_phab , color=CCAA), size=0.25,alpha=0.7) +
  geom_text(aes(Date+3,confirmados_phab,label=CCAA, color=CCAA), size=3, data=tot_casos_sel) +
  #scale_color_manual(values=getPalette(20)) +
  scale_color_brewer(palette = "Set1") +
  theme(legend.position = "none") #

plotly::ggplotly(p,dynamicTicks =TRUE)



```

Escala logarítmica

```{r}
ggplot(data_casos_ccaa) + geom_point(aes(Date,num_phab, color=CCAA)) + 
  geom_line(aes(Date,num_phab, color=CCAA), size=0.25,alpha=0.7) + 
  ggrepel::geom_text_repel(aes(Date+3,confirmados_phab,label=CCAA, color=CCAA), size=3, data=tot_casos_sel, segment.alpha = 0,min.segment.length = 0.5) +
  scale_y_log10() + theme(legend.position = "none") +
  scale_color_brewer(palette = "Set1") +
  theme(legend.position = "none") + labs(y="Confirmados por 100.000 hab") 
  #coord_cartesian(ylim=c(1,300))
```

Casos nuevos

```{r}
ggplot(data_new_casos_ccaa) + geom_point(aes(Date,num_phab, color=CCAA)) + 
  geom_line(aes(Date,num_phab, color=CCAA), size=0.25,alpha=0.7) + 
  ggrepel::geom_text_repel(aes(Date+3,num_phab,label=CCAA, color=CCAA), size=3, data=last_new_ccaa, segment.alpha = 0,min.segment.length = 0.5) +
  scale_y_log10() + theme(legend.position = "none") +
  scale_color_brewer(palette = "Set1") +
  theme(legend.position = "none") + labs(y="Confirmados por 100.000 hab") 

```

Casos diarios: Nuevos confirmados - Nuevos recuperados

```{r, eval=FALSE}
# Cambiar para CCAA

ggplot(data_new_all_ccaa %>% filter(activos>0, Date>"2020-03-01")) + 
  geom_line(aes(Date,activos, group=1), alpha=0.7) +
  geom_smooth(aes(Date,activos, color=CCAA), alpha=1, size=0.75, se=FALSE) +  
  facet_wrap(~CCAA, scales="free_y") + labs(y="Nuevos Casos - Nuevos Recuperados (diarios)") + 
  theme(axis.text.x = element_text(angle = 30, hjust = 1)) + theme(legend.position = "none") +
  scale_color_brewer(palette = "Set1")
```

### Ajustes y tiempos de duplicación


```{r}
cut_fit_date = Sys.Date()- 5

data_casos_ccaa <- data_casos_ccaa %>%  mutate(t=as.numeric(Date-min(Date)))

exp_fits <- data_casos_ccaa %>% filter(Date>=cut_fit_date) %>% #filter(num>50) %>%  
  nest(-CCAA) %>% group_by(CCAA) %>%  
      mutate(exp_mod = purrr::map(data, ~lm(log(num_phab)~t, data= .))) 

coefs<- exp_fits %>%
      mutate(tidied = purrr::map(exp_mod,broom::tidy)) %>% 
      unnest(tidied) %>% select(CCAA,term, estimate)

dupl_times<- coefs %>% filter(term =="t") %>% 
        mutate(dupl_t = log(2)/estimate)

dupl_times

```

```{r}
fitted_data = exp_fits %>% 
  mutate(fitted=map(exp_mod,augment)) %>%  unnest(fitted) %>% mutate(fitted=exp(.fitted)) %>% 
  select(CCAA,t, fitted) %>%  mutate(Date=min(data_casos_ccaa$Date) + t)


ggplot(data_casos_ccaa) + geom_point(aes(Date,num_phab, color=CCAA)) + 
      geom_line(aes(Date,fitted, color=CCAA), data=fitted_data) +
      ggrepel::geom_text_repel(aes(Date+5,confirmados_phab,label=CCAA, color=CCAA), 
                               size=3, data=tot_casos_sel, segment.alpha = 0,min.segment.length = 0.5) +
  theme(legend.position = "none") +
  scale_color_brewer(palette = "Set1") +
  theme(legend.position = "none") +
  coord_cartesian(ylim=c(1,max(data_casos_ccaa$num_phab)))
  
```


```{r}
ggplot(data_casos_ccaa) + geom_point(aes(Date,num_phab, color=CCAA)) + 
      geom_line(aes(Date,fitted, color=CCAA), data=fitted_data) +
      ggrepel::geom_text_repel(aes(Date+5,confirmados_phab,label=CCAA, color=CCAA), 
                               size=3, data=tot_casos_sel, segment.alpha = 0,min.segment.length = 0.5) +
  scale_y_log10() + 
  theme(legend.position = "none") +
  scale_color_brewer(palette = "Set1") +
  theme(legend.position = "none") +
  coord_cartesian(ylim=c(1,max(data_casos_ccaa$num_phab)))
```

### Tiempo duplicación dinámico

```{r}
fit_window=7 # days
first_date = as.Date("2020-03-10")

fit_dates <- unique(data_casos_ccaa$Date[data_casos_ccaa$Date>=first_date])

din_dupl_ccaa <- data.frame(fdate= fit_dates) %>% group_by(fdate) %>% do({
    fd = .$fdate
    df <- data_casos_ccaa %>%filter(Date<=fd, Date>fd-fit_window, num>0)
    res <- df %>% nest(-CCAA) %>% group_by(CCAA) %>%  
      mutate(exp_mod = purrr::map(data, ~lm(log(num)~t, data= .))) %>%
      mutate(tidied = purrr::map(exp_mod,broom::tidy)) %>% 
      unnest(tidied) %>% select(CCAA,term, estimate, std.error) %>% filter(term=="t") %>% 
        mutate(dupl_t = log(2)/estimate)
})

ggplot(din_dupl_ccaa) + geom_point(aes(fdate,dupl_t, color=CCAA)) +
  geom_smooth(aes(fdate,dupl_t, color=CCAA), se=FALSE, size=0.5, alpha=0.2, span=0.5) +
  ggrepel::geom_text_repel(aes(fdate+2,dupl_t,label=CCAA, color=CCAA), 
                           size=3, data=din_dupl_ccaa %>% ungroup %>% filter(fdate==max(fdate)) , 
                           segment.alpha = 0,min.segment.length = 0.5) +
  theme(legend.position="none") +
  scale_color_brewer(palette="Set1") +
  labs(y="Tiempo de duplicación de casos (dias)", title="Tiempos de duplicacion en ventana movil de una semana antes de la fecha")

```

##  Fallecidos

### Mapa 

```{r}

pal_muertes <- colorNumeric("YlOrRd",shpmap$muertes_phab,
                        na.color = "transparent")


popup_dat <- paste0("<strong>CCAA: </strong>", 
                            shpmap$CCAA,
                            "<br><strong>Casos: </strong>", 
                            round(shpmap$confirmados,0), 
                            "<br><strong>Casos x 100000 hab </strong>",
                            round(shpmap$confirmados_phab,1),
                            "<br><strong>Fallecidos: </strong>", 
                            round(shpmap$muertes,0), 
                            "<br><strong>Fallecidos x 100000 hab </strong>",
                            round(shpmap$muertes_phab,1))


leaflet(data=shpmap) %>%
  addTiles() %>% 
  #setView(lng=-1.5, lat=39.85, zoom = 06) %>% 
  addPolygons(fillColor = ~pal_muertes(muertes_phab),
              stroke = TRUE, color="grey",weight = 1, fillOpacity =0.8, popup = popup_dat) %>% 
  leaflet::addLegend(position="bottomright", title="Fallecidos <br> x 100000 hab", pal=pal_muertes,values = shpmap$muertes_phab)


```



### Evolución fallecidos

```{r}
ggplot(data_ccaa %>% filter(tipo=="muertes")) + geom_line(aes(Date, num_phab)) + facet_wrap(~CCAA)
```


```{r}

data_muertes_ccaa = data_ccaa %>% filter(Date>=date_ini , tipo=="muertes", (cod_ine %in% sel_ccaa))
tot_muertes_sel <- tot_ccaa %>% filter(cod_ine %in% sel_ccaa)



p <- ggplot(data_muertes_ccaa) + geom_point(aes(Date,num_phab, color=CCAA)) + 
  geom_line(aes(Date,num_phab , color=CCAA), size=0.25,alpha=0.7) +
  ggrepel::geom_text_repel(aes(Date+3,muertes_phab,label=CCAA, color=CCAA), size=3, data=tot_muertes_sel, segment.alpha = 0,min.segment.length = 0.5) +
  #scale_color_manual(values=getPalette(8)) +
  scale_color_brewer(palette = "Set1") +
  theme(legend.position = "left") #
plotly::ggplotly(p)



```

```{r}
ggplot(data_muertes_ccaa) + geom_point(aes(Date,num_phab, color=CCAA)) + 
  geom_line(aes(Date,num_phab , color=CCAA), size=0.25,alpha=0.7) +
  ggrepel::geom_text_repel(aes(Date+3,muertes_phab,label=CCAA, color=CCAA), size=3, data=tot_muertes_sel, segment.alpha = 0,min.segment.length = 0.5) +
  #scale_color_manual(values=getPalette(8)) +
  scale_color_brewer(palette = "Set1") +
  theme(legend.position = "none") +
  scale_y_log10() +
  coord_cartesian(ylim=c(1,max(data_muertes_ccaa$num_phab)))


```

Nuevos fallecidos

```{r}
ggplot(data_new_muertes_ccaa %>% filter( Date>"2020-03-01")) + 
  geom_line(aes(Date,num, group=1), alpha=0.7) +
  geom_smooth(aes(Date,num, color=CCAA), alpha=1, size=0.75, se=FALSE) +  
  facet_wrap(~CCAA, scales="free_y") + labs(y="Nuevos fallecimientos (diarios)") + 
  theme(axis.text.x = element_text(angle = 30, hjust = 1)) + theme(legend.position = "none") +
  scale_color_brewer(palette = "Set1")
```


### Ajustes y tiempos de duplicación


```{r}
cut_fit_date =Sys.Date() -5
data_muertes_ccaa <- data_muertes_ccaa %>%  mutate(t=as.numeric(Date-min(Date)))

exp_fits_muertes <- data_muertes_ccaa %>% filter(Date>=cut_fit_date, num>0) %>% #filter(num>50) %>%  
  nest(-CCAA) %>% group_by(CCAA) %>%  
      mutate(exp_mod = purrr::map(data, ~lm(log(num_phab)~t, data= .))) 

coefs_muertes<- exp_fits_muertes %>%
      mutate(tidied = purrr::map(exp_mod,broom::tidy)) %>% 
      unnest(tidied) %>% select(CCAA,term, estimate)

dupl_times_muertes<- coefs_muertes %>% filter(term =="t") %>% 
        mutate(dupl_t = log(2)/estimate)

dupl_times_muertes

```

```{r}
fitted_data_muertes = exp_fits_muertes %>% 
  mutate(fitted=map(exp_mod,augment)) %>%  unnest(fitted) %>% mutate(fitted=exp(.fitted)) %>% 
  select(CCAA,t, fitted) %>%  mutate(Date=min(data_muertes_ccaa$Date) + t)


ggplot(data_muertes_ccaa) + geom_point(aes(Date,num_phab, color=CCAA)) + 
      geom_line(aes(Date,fitted, color=CCAA), data=fitted_data_muertes) +
      ggrepel::geom_text_repel(aes(Date+5,muertes_phab,label=CCAA, color=CCAA), 
                               size=3, data=tot_muertes_sel, segment.alpha = 0,min.segment.length = 0.5) +
  theme(legend.position = "none") +
  scale_color_brewer(palette = "Set1") +
  theme(legend.position = "none") +
  coord_cartesian(ylim=c(1,max(data_muertes_ccaa$num_phab)))
  

```

```{r}
ggplot(data_muertes_ccaa) + geom_point(aes(Date,num_phab, color=CCAA)) + 
      geom_line(aes(Date,fitted, color=CCAA), data=fitted_data_muertes) +
      ggrepel::geom_text_repel(aes(Date+5,muertes_phab,label=CCAA, color=CCAA), 
                               size=3, data=tot_muertes_sel, segment.alpha = 0,min.segment.length = 0.5) +
  scale_y_log10() + labs(y="Fallecidos por 100000 hab") +
  theme(legend.position = "none") +
  scale_color_brewer(palette = "Set1") +
  theme(legend.position = "none") +
  coord_cartesian(ylim=c(1,max(data_muertes_ccaa$num_phab)))

```


### Tiempo duplicación dinámico

```{r}
fit_window=7 # days
first_date = as.Date("2020-03-10")

fit_dates <- unique(data_muertes_ccaa$Date[data_muertes_ccaa$Date>=first_date])

din_dupl_muertes_ccaa <- data.frame(fdate= fit_dates) %>% group_by(fdate) %>% do({
    fd = .$fdate
    df <- data_muertes_ccaa %>%filter(Date<=fd, Date>fd-fit_window, num>0)
    res <- df %>% nest(-CCAA) %>% group_by(CCAA) %>%  
      mutate(exp_mod = purrr::map(data, ~lm(log(num)~t, data= .))) %>%
      mutate(tidied = purrr::map(exp_mod,broom::tidy)) %>% 
      unnest(tidied) %>% select(CCAA,term, estimate, std.error) %>% filter(term=="t") %>% 
        mutate(dupl_t = log(2)/estimate)
})

ggplot(din_dupl_muertes_ccaa) + geom_point(aes(fdate,dupl_t, color=CCAA)) +
  geom_smooth(aes(fdate,dupl_t, color=CCAA), se=FALSE, size=0.5, alpha=0.2, span=0.5) +
  ggrepel::geom_text_repel(aes(fdate+2,dupl_t,label=CCAA, color=CCAA), 
                           size=3, data=din_dupl_muertes_ccaa %>% ungroup %>% filter(fdate==max(fdate)) , 
                           segment.alpha = 0,min.segment.length = 0.5) +
  theme(legend.position="none") +
  scale_color_brewer(palette="Set1") +
  labs(y="Tiempo de duplicación de fallecidos (dias)", title="Tiempos de duplicación en ventana movil de una semana antes de la fecha")

```

# Italia


```{r}
url_base <- "https://raw.githubusercontent.com/pcm-dpc/COVID-19/master/dati-regioni"
data_italia <- readr::read_csv(paste0(url_base,"/dpc-covid19-ita-regioni.csv"))
last_italia <- data_italia %>% group_by(denominazione_regione) %>% filter(data==max(data))

last_italia %>% arrange(desc(totale_casi)) %>% mutate(mort_rate=deceduti/totale_casi,rec_rate=dimessi_guariti/totale_casi) %>% 
  select(denominazione_regione, totale_casi, deceduti, dimessi_guariti, mort_rate, rec_rate, everything())
```





# Referencias interesantes

Modelos epidemiológicos formulados en base a ecuaciones diferenciales que describen la dinámica de la infección pueden encontrase aquí:

- <https://staff.math.su.se/hoehle/blog/2020/03/16/flatteningthecurve.html>

- <https://timchurches.github.io/blog/posts/2020-03-10-modelling-the-effects-of-public-health-interventions-on-covid-19-transmission-part-1/>

- <https://blog.ephorie.de/epidemiology-how-contagious-is-novel-coronavirus-2019-ncov>

Otros basados, además en datos de movilidad geográfica:

- <https://covid-19-risk.github.io/map/>
- <https://covid-19-sds.github.io/>

Estimación de sub-detección de casos

- <https://cmmid.github.io/topics/covid19/severity/global_cfr_estimates.html>


